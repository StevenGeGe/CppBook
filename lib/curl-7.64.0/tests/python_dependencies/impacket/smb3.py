...
class SMB3:
        ...
        if session is None:
            ...
            if UDP:
                self._NetBIOSSession = nmb.NetBIOSUDPSession(my_name, self._Connection['ServerName'], remote_host,
                                                             host_type, sess_port, self._timeout)
            else:
    ...
    def sendSMB(self, packet):
        ...
        if (self._Session['SessionFlags'] & SMB2_SESSION_FLAG_ENCRYPT_DATA) or (
                packet['TreeID'] != 0 and self._Session['TreeConnectTable'][packet['TreeID']]['EncryptData'] is True):
    ...
    def kerberosLogin(self, user, password, domain='', lmhash='', nthash='', aesKey='', kdcHost='', TGT=None, TGS=None):
        ...
        if TGS is None:
            serverName = Principal('cifs/%s' % (self._Connection['ServerName']),
        ...
        if ans.isValidAnswer(STATUS_SUCCESS):
            ...
            if self._Session['SigningRequired'] is True and self._Connection['Dialect'] == SMB2_DIALECT_30:
                self._Session['SigningKey'] = crypto.KDF_CounterMode(self._Session['SessionKey'], "SMB2AESCMAC\x00",
                                                                     "SmbSign\x00", 128)
            ...
            if self._Connection['Dialect'] == SMB2_DIALECT_30:
                self._Session['ApplicationKey'] = crypto.KDF_CounterMode(self._Session['SessionKey'], "SMB2APP\x00",
                                                                         "SmbRpc\x00", 128)
                self._Session['EncryptionKey'] = crypto.KDF_CounterMode(self._Session['SessionKey'], "SMB2AESCCM\x00",
                                                                        "ServerIn \x00", 128)
                self._Session['DecryptionKey'] = crypto.KDF_CounterMode(self._Session['SessionKey'], "SMB2AESCCM\x00",
                                                                        "ServerOut\x00", 128)
    ...
    def login(self, user, password, domain='', lmhash='', nthash=''):
        ...
        if ans.isValidAnswer(STATUS_MORE_PROCESSING_REQUIRED):
            ...
            if exportedSessionKey is not None:
                ...
                if self._Session['SigningRequired'] is True and self._Connection['Dialect'] == SMB2_DIALECT_30:
                    self._Session['SigningKey'] = crypto.KDF_CounterMode(exportedSessionKey, "SMB2AESCMAC\x00",
                                                                         "SmbSign\x00", 128)
            ...
            try:
                if packet.isValidAnswer(STATUS_SUCCESS):
                    ...
                    if self._Session['SigningRequired'] is True:
                        self._Session['SigningActivated'] = True
                    if self._Connection['Dialect'] == SMB2_DIALECT_30:
                        self._Session['ApplicationKey'] = crypto.KDF_CounterMode(exportedSessionKey, "SMB2APP\x00",
                                                                                 "SmbRpc\x00", 128)
                        self._Session['EncryptionKey'] = crypto.KDF_CounterMode(exportedSessionKey, "SMB2AESCCM\x00",
                                                                                "ServerIn \x00", 128)
                        self._Session['DecryptionKey'] = crypto.KDF_CounterMode(exportedSessionKey, "SMB2AESCCM\x00",
                                                                                "ServerOut\x00", 128)
    ...
    def connectTree(self, share):
        ...
        if packet.isValidAnswer(STATUS_SUCCESS):
            ...
            if (treeConnectResponse[
                    'Capabilities'] & SMB2_SHARE_CAP_CONTINUOUS_AVAILABILITY) == SMB2_SHARE_CAP_CONTINUOUS_AVAILABILITY:
                treeEntry['IsCAShare'] = True

            if self._Connection['Dialect'] == SMB2_DIALECT_30:
                if (self._Connection['SupportsEncryption'] is True) and ((treeConnectResponse[
        ...
        if ans.isValidAnswer(STATUS_SUCCESS):
            ...
            if bytesWritten < bytesToWrite:
                bytesWritten += self.write(treeId, fileId, data[bytesWritten:], offset + bytesWritten,
                                           bytesToWrite - bytesWritten, waitAnswer)
            return bytesWritten

    def queryDirectory(self, treeId, fileId, searchString='*', resumeIndex=0, informationClass=FILENAMES_INFORMATION,
                       maxBufferSize=None, enumRestart=False, singleEntry=False):
        if self._Session['TreeConnectTable'].has_key(treeId) is False:
            raise SessionError(STATUS_INVALID_PARAMETER)
        if self._Session['OpenTable'].has_key(fileId) is False:
            raise SessionError(STATUS_INVALID_PARAMETER)
    ...
    def ioctl(self, treeId, fileId=None, ctlCode=-1, flags=0, inputBlob='', maxInputResponse=None,
              maxOutputResponse=None, waitAnswer=1):
        if self._Session['TreeConnectTable'].has_key(treeId) is False:
            raise SessionError(STATUS_INVALID_PARAMETER)
        if fileId is None:
            fileId = '\xff' * 16
    ...
    def queryInfo(self, treeId, fileId, inputBlob='', infoType=SMB2_0_INFO_FILE, fileInfoClass=SMB2_FILE_STANDARD_INFO,
    ...
    def setInfo(self, treeId, fileId, inputBlob='', infoType=SMB2_0_INFO_FILE, fileInfoClass=SMB2_FILE_STANDARD_INFO,
    ...
    def listPath(self, shareName, path, password=None):
        ...
        try:
            ...
            fileId = self.create(treeId, ntpath.dirname(path), FILE_READ_ATTRIBUTES | FILE_READ_DATA,
    ...
    def remove(self, shareName, pathName, password=None):
        ...
        fileId = None
        try:
            fileId = self.create(treeId, pathName, DELETE | FILE_READ_ATTRIBUTES, FILE_SHARE_DELETE,
                                 FILE_NON_DIRECTORY_FILE | FILE_DELETE_ON_CLOSE, FILE_OPEN, 0)
        finally:
            if fileId is not None:
                self.close(treeId, fileId)
            self.disconnectTree(treeId)
    ...
    def storeFile(self, shareName, path, callback, mode=FILE_OVERWRITE_IF, offset=0, password=None,
                  shareAccessMode=FILE_SHARE_WRITE):
        # ToDo: Handle situations where share is password protected
        path = string.replace(path, '/', '\\')
        path = ntpath.normpath(path)
        if len(path) > 0 and path[0] == '\\':
    ...
    def waitNamedPipe(self, treeId, pipename, timeout=5):
        ...
        return self.ioctl(treeId, None, FSCTL_PIPE_WAIT, flags=SMB2_0_IOCTL_IS_FSCTL, inputBlob=pipeWait,
    ...
    def TransactNamedPipe(self, tid, fid, data, noAnswer=0, waitAnswer=1, offset=0):
        return self.ioctl(tid, fid, FSCTL_PIPE_TRANSCEIVE, SMB2_0_IOCTL_IS_FSCTL, data, maxOutputResponse=65535,
                          waitAnswer=noAnswer | waitAnswer)
